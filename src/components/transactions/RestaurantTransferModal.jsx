...
import { Button, Form, Input, Modal, Result, Select, Spin, notification } from 'antd';
import React, { useEffect, useState } from 'react';

import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { _account_number_verification, _bank_verification, _calc_commission, _get_all_banks, _get_beneficiaries, _get_profile, _get_user_account_history, _payout, _payout_internal, _verify_pin } from '../../common/axios_services';
import { updateCount } from '../../slices/updateCountSlice';

export default function RestaurantTransferModal({ onComplete }) {
  ...
  const wallet_validation = async ({ accountNo }) => {
    ...
    try {
      const accOwner = await _account_number_verification({
      ...
      form.setFieldsValue({
        accName: accOwner?.data?.data?.account_name,
      });
      ...
      >
        {profileData && verifiedPin ? (
          <div style={{ height: '400px', overflowY: 'auto' }}>
            <h2 style={{ fontSize: '2.5rem' }}>Transfer to bank</h2>
